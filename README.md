...
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/Apoorv-Tripathi/SkillSetu)
...
Seed the database with comprehensive demo records:
```bash
npm run seed
```
...
SkillSetu comes pre-configured with a `render.yaml` Blueprint for 1-click deployment on [Render](https://render.com).

### Option 1: 1-Click Blueprint (Recommended)
Click the badge below or visit the direct deployment link:

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/Apoorv-Tripathi/SkillSetu)

**Direct Link:**  
`https://render.com/deploy?repo=https://github.com/Apoorv-Tripathi/SkillSetu`

During setup, Render will prompt for:
1. `MONGODB_URI`: Your MongoDB Atlas connection string.
2. `JWT_SECRET`: A secure random string (auto-generated by Render if left blank).
...
1. **Backend Web Service**:
   ...
   - **Environment Variables**: `PORT=5050`, `MONGODB_URI`, `JWT_SECRET`, `CLIENT_ORIGIN=<frontend-url>`
2. **Frontend Static Site**:
   - **Root Directory**: `frontend`
   - **Build Command**: `npm install && npm run build`
   - **Publish Directory**: `dist`
   - **SPA Rewrite Rule**: `/*` → `/index.html`
   - **Environment Variables**: `VITE_API_BASE_URL=<backend-url>/api`
